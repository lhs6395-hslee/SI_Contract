...
다음 값은 코드 현행값이며 권위 출처(공문/경영 결정)가 미확정 상태이다. 운영팀 인터뷰 확정 전까지 "잠정"으로 사용.
...
1. **직급 단가표 3중 충돌** (구 FR-09 이동)
   ...
   - **해소 방법**: 운영팀(FDE) 인터뷰에서 적용 근거 직급단가표 원본 문서 확인 후 단일 출처로 확정.
...
2. **상여금 계산 공식 충돌** (구 FR-06 이동)
   ...
   - **해소 방법**: 운영팀 인터뷰에서 /9 기준의 "명절까지 투입개월" 정의와 "전액" 정의 중 어느 쪽이 기준인지 확정.
