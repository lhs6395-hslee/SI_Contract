...
- **SC-001**: **검증 사례셋(정상 LLM 응답이 보장된 실제 집행계획서 양식 기반 테스트 문서 집합, 예: GS EPS·퀘이사존 양식) 기준**으로, `/api/extract` 호출 시 응답 HTTP 상태 코드가 `200`이고 반환 JSON에 `projectName` 키가 존재하는 비율이 **100%** (fallback `{"error":"추출 실패"}` 반환 비율 0%). 깨진 LLM 응답이 입력인 경우는 FR-015a/b 범위이므로 이 기준 집합에서 제외한다.
...
1. **추출 정확도 SC 목표** (SC-005): 검증 사례셋 기반 베이스라인 측정 후 수치 확정 필요. 현재 단일 기준 출처 없음.
