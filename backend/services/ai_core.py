"""AI Core — backend·ai-service 공유 AI 로직 (자기완결).

프롬프트/파싱/category 정규화/Vision 멀티모달/모델 라우팅/Bedrock 호출을 한곳에 모아
backend(모놀리스)와 ai-service(MSA)가 **동일 코드**를 쓰도록 한다. 프롬프트 drift 차단.

이 모듈은 다른 backend 모듈에 의존하지 않는다(ai-service에서 단독 복사 사용 가능).
"""
...
# ai-service가 독립 동작하도록 라우팅을 ai_core에 내장(별도 llm_gateway 의존 제거).
HAIKU_MODEL = os.getenv("HAIKU_MODEL", "global.anthropic.claude-haiku-4-5-20251001-v1:0")
DEFAULT_MODEL = os.getenv("DEFAULT_MODEL", BEDROCK_MODEL_ID)
...
def route_model(task_type: str, user_id: str = "default") -> str:
    ...
    _logger.debug("LLM route: task=%s tier=%s model=%s user=%s", task_type, tier, model, user_id)
    return model
...
def get_bedrock_client():
    """싱글톤 Bedrock 클라이언트. credential 미설정 시 AIUnavailableError."""
    global _client
    if _client is None:
        try:
...
def invoke_bedrock(
    ...
    """공용 Bedrock invoke — 모델 라우팅 + Vision 멀티모달.

    images: base64-encoded PNG 목록. 지정 시 텍스트+이미지 멀티모달 메시지로 전송.
    """
    if not model_id:
    ...
    body_dict = {
        ...
        "temperature": temperature,  # 추출/분류 결정론화 (기본 0)
...
def _call_claude(prompt: str, max_tokens: int = 2048, task_type: str = "sonnet", user_id: str = "default", images: list[str] | None = None) -> str:
    """Bedrock Claude 단일 호출 — 모델 라우팅 경유. images 지정 시 Vision 멀티모달."""
...
- "bonus"    → 상여금 (원가명세의 상여 라인)
- "wage"     → 임금 (자사 직접인력 임금 금지 — 급료와 함께 staffPlan 계산)
- "welfare"  → 복리후생비 (원가명세의 복리후생 라인)
- "travel"   → 여비/출장비
- "vehicle"  → 차량유지비
...
규칙:
- **자사 직접인력의 급료·임금(labor/wage)은 추출 금지** — 인원(staffPlan)+사내 직급단가표가
  계산한다. 견적서의 PM/SA 등 자사 인력 라인을 labor/wage로 넣지 마세요.
  (단 **상여(bonus)·복리후생(welfare)는 원가명세에 별도 라인이 있으면 추출하세요** — 빌더가
  계산하지 않는 항목임. 외주/도급 노무비만 labor, 협력사 수수료는 fee.)
...
def extract_section(section: str, documents: list[dict]) -> dict:
    """섹션 키로 granular 추출 디스패치 (costs/people/schedule/rates/org)."""
    fn = {
        "costs": extract_costs,
...
# ─── 챗봇 (순수 추론 — 컨텍스트 구성은 호출측 책임) ─────────

def chat_complete(message: str, system: str, max_tokens: int = 1024, user_id: str = "default") -> dict:
    """프로젝트 컨텍스트(system)와 사용자 메시지로 1턴 추론. invoke_bedrock 결과 그대로 반환."""
