...
function TabBasic({ onManualEdit, verifiedFields }: { onManualEdit: (key: string) => void; verifiedFields: Set<string> }) {
  ...
  // 생성시각: 서버 UTC ISO → KST(UTC+9, 한국 DST 없음) 컴팩트 표기 "YYYY.MM.DD HH:mm"
  const createdAtKST = (() => {
    if (!extractedData?.createdAt) return "-";
    const t = new Date(extractedData.createdAt).getTime();
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-[100px_1fr]">
              ...
              <div className="text-sm text-muted-foreground py-3 border-b whitespace-nowrap">생성시각 <span className="text-[10px]">(읽기전용)</span></div>
              <div className="text-sm py-3 border-b tabular-nums whitespace-nowrap">{createdAtKST}</div>
            </div>
          </CardContent>
        </Card>
      </div>
