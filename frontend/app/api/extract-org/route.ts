...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetchBackend("/api/extract-org", { method: "POST", body: proxyForm });
    const result = await res.json();
    return NextResponse.json(result, { status: res.status });
  } catch (err) {
    console.error("[extract-org] proxy error:", err);
    return NextResponse.json({ organization: [], error: `서버 오류: ${err instanceof Error ? err.message : "unknown"}` }, { status: 500 });
  }
}
